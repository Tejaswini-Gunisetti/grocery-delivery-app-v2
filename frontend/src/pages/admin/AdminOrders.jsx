...
const AdminOrders = () => {
  ...
  return (
    <div className="p-10 max-md:p-6 min-h-screen">
      ...
      <h2 className="text-[20px] font-bold text-gray-800 mb-8 uppercase tracking-wide">Customer Orders</h2>
      ...
      {orders.length > 0 ? (
        <div className="flex flex-col gap-6">
          {orders.map(order => (
            ...
            >
              <FiPackage className="text-primary shrink-0 animate-bounce" size={32} />

              <div className="flex-[2] min-w-0">
                <div className="font-bold text-gray-800 mb-2 leading-relaxed">
                  {order.items.map(i => `${i.name} x ${(i.quantity || i.qty || 1)}`).join(', ')}
                </div>
                <div className="text-gray-600 font-bold mb-1.5">{order.customer?.name || "Customer"}</div>
                <div className="text-gray-500 text-[13px] leading-relaxed">
                  <p>{order.customer?.address}</p>
                  <p className="mt-1 font-semibold text-gray-700">{order.customer?.phone}</p>
                </div>
              </div>

              <div className="flex-1 text-gray-600 leading-[1.8] font-semibold max-md:mt-3">
                <p>Items: <span className="text-gray-800 font-bold">{order.items.length}</span></p>
                <p>Method: <span className="text-gray-800 font-bold">{order.method || order.payment}</span></p>
                <p>Status:
                  <span className={`ml-2 px-2.5 py-0.5 border text-xs rounded-full font-bold inline-block capitalize ${getStatusBgColor(order.status)}`}>
                    {order.status}
                  </span>
                </p>
                <p>Date: <span className="text-gray-800 font-bold">{new Date(order.createdAt).toLocaleDateString('en-US')}</span></p>
              </div>
        ...
        <div className="text-center py-20 bg-gray-50 border border-dashed border-gray-200 rounded-2xl">
          <p className="text-gray-500 font-medium">No customer orders available.</p>
        </div>
      )}
    </div>
  );
